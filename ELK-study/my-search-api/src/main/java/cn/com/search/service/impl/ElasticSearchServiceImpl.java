...
public class ElasticSearchServiceImpl implements ElasticSearchService {
    ...
    public Boolean searchBulkIn(List<Map<String, Object>> mapList, String index, String type) throws Exception {
        try {
            // 获取插入数据的流, 一个request的流
            BulkRequestBuilder bulkRequest = elasticSearchManger.client.prepareBulk();
            for (Map<String, Object> map : mapList) {
                // 在 elasticsearch里用的是json格式, 所以需要将数据转为json格式
                String content = JSONObject.toJSONString(map);
                System.out.println(content);
                bulkRequest.add(elasticSearchManger.client.prepareIndex(index, type, map.get("id").toString())
                        .setSource(map));
            }
			// 真实写数据
    ...
    public List<BookSearchResultVo> queryDocumentByParam(String indices, String type, BookSearchParam param) {
        ...
        // 分页：注意es其实是内存分页
        // 比如你分页10条，查第10页，会把前10也全部拿出来放到内存中，给你取你要的后面的数据.
        // 不要查的太多，防止被人参数注入，内存爆炸，最多不要超过1000页. (通过 if 判断, 不允许查询1000页后的数据)
        builder.setFrom(param.getPageIndex()).setSize(param.getPageSize());
        // 结合权重排序 需要自己先离线计算好分值。然后在使用es里面的sum加权函数就可以了
        builder.addSort("price", SortOrder.ASC);
        // es中设置高亮，将某些查询出来的结果变为红色、加粗等
