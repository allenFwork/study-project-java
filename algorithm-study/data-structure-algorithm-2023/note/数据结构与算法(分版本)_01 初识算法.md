...
public static int binarySearch(int[] a, int target) {
    ...
    // 假设循环L次：元素在最左边，第一个if循环执行L次；元素在最右边，if和else if 循环执行各L次，总共执行2L次
    while (i <= j) {
        int m = (i + j) >>> 1;			// >>> 位运算符，无符号右移；>> 有符号右移运算符
        if (target < a[m]) {            // 在左边
            j = m - 1;
...
<img src=".\imgs\image-20221108103846566.png" alt="image-20221108103846566" style="zoom:100%;" />
...
* 表达式中相乘的常量，可以省略，如
    * $f(n) = 100*n^2$ 中的 $100$
* 多项式中数量规模更小（低次项）的表达式可以省略，如
    * $f(n)=n^2+n$ 中的 $n$
    * $f(n) = n^3 + n^2$ 中的 $n^2$
* 不同底数的对数，渐进上界可以用一个对数函数 $\log n$ 表示
    * 例如： $log_{2}(n)$ 可以替换为 $log_{10}(n)$，因为 $log_2(n) = \frac{log_{10}(n)}{log_{10}(2)}$，相乘的常量 $\frac{1}{log_{10}(2)}$ 可以省略 
* 类似的，对数的常数次幂可省略
    * 如： $log(n^c) = c * log(n)$
...
<img src=".\imgs\image-20221108114915524.png" alt="image-20221108114915524" style="zoom:100%;" />
...
2. 不奢望循环内通过 $m$ 找出目标, 缩小区间直至剩 1 个, 剩下的这个可能就是要找的（通过 $i$）
    ...
    * 写成 $j-i > 1$ 与 $i < j$ 相比，可以少比较一次，最后一次i位置的值可以在后面的 a[i] == target 中比较
...
5. 时间复杂度 $\Theta(log(n))$
    ...
    - 此时最好和最差的情况都是 $O(log(n))$ ，所以时间复杂度可以写成是 $\Theta(log(n))$

    - 原来基础版的最好情况是 $O(1)$ ，最差情况是 $O(log(n))$ ，不能写成 $\Theta(log(n))$



### 1.5.2 Java 版

jdk自带的二分查找，在 java.util.Arrays类里面

<img src=".\imgs\2024-12-13-17-25-50-image.png" alt="image-2024-12-13-17-25-50-image" style="zoom:100%;" />
...
<img src="./imgs/2024-12-13-18-16-22-image.png" alt="2024-12-13-18-16-53-image" style="zoom:100%;" />

<img src="./imgs/2024-12-13-18-16-53-image.png" alt="2024-12-13-18-16-53-image" style="zoom:100%;" />

几个名词

<img src="./imgs/image-20221125174155058.png" alt="image-20221125174155058" style="zoom:100%;" />
...
### 1.6.5 E03. 搜索开始结束位置-Leetcode 34
