...
public class AsmEnhancer {
    ...
    // 传入字节码文件的字节数组，对其进行修改，然后返回修改后的字节码文件对应的字节数组
    public static byte[] enhanceClass(byte[] bytes) {
        ...
        ClassVisitor classVisitor = new ClassVisitor(ASM7, classWriter) {
            ...
            public MethodVisitor visitMethod(int access, String name, String descriptor, String signature, String[] exceptions) {
                // methodVisitor就是原来字节码文件中对应的每一个方法
                MethodVisitor methodVisitor = super.visitMethod(access, name, descriptor, signature, exceptions);
                // 自定义的MethodVisitor对象，其中编写了对每个方法的添加逻辑
...
class MyMethodVisitor extends MethodVisitor {
    ...
    // 字节码文件中每个方法执行一开始，调用System.out.println（"开始执行"）方法
    @Override
    ...
    // 字节码文件中每个方法返回时，执行System.out.println("结束执行")
    @Override
