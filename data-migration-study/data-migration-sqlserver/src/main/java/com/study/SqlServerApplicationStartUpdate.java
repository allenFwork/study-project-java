    ...
    static class DataThread implements Callable {
        ...
        public Object call() throws Exception {
            ...
            try {
                ...
                // 每页查询数量
                long pageSizeByCount = pageSize / 10000;
                // 剩余数量
                long leaveCount = pageSize % 10000;
                String sql1 = "SELECT * FROM Tactical_Offline..sc_ship_part_cml_copy ORDER BY BU OFFSET ? rows fetch next ? rows only";
                String sql2 = "INSERT INTO HAWK_TACTICAL..sc_ship_part_cml_copy (bu,product_group,brand,plant_code,geo,subgeo,site_id,family,product,product_qty,sbb,sbb_qty,part,part_qty,ship_date,sys_created_by,update_timestamp,itemgroup,sci_site_id,product_line,ludp_update_timestamp,segment) " +
                        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                ...
                for (int i = 0; i < pageSizeByCount; i++) {
                    ...
                    preparedStatement1.setLong(2, 10000);
                    ...
                    pageNoByCount = pageNoByCount + 10000;
                }
                if (leaveCount > 0) {
                    preparedStatement1 = connection1.prepareStatement(sql1);
                    preparedStatement1.setLong(1, pageNoByCount);
