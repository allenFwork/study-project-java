...
- tokenizer filter：<font color=red>将tokenizer输出的词条做进一步处理</font>。例如大小写转换、同义词处理、拼音处理等
...
```json
PUT /test                 // 创建test的库，库中存放以下配置的分词器
{
  "settings": {
    "analysis": {
      "analyzer": {                    // 自定义分词器
        "my_analyzer": {               // 分词器名称
          "tokenizer": "ik_max_word",  // 实际分词的功能
          "filter": "py"               // 将tokenizer输出的词条做进一步处理,此处定义了使用名为 py 的过滤器
        }
      },
      "filter": {                      // 自定义tokenizer filter
        "py": {                        // 过滤器名称
          "type": "pinyin",            // 过滤器类型，实际的过滤器功能，这里是pinyin
  ...
  "mappings": {
    "properties": {
      "name": {
        "type": "text",                 // 该属性name对应的类型，text 文本类型
        "analyzer": "my_analyzer",      // 使用自己定义的分词器，在创建文档数据时，使用这个分词器处理name属性数据
        "search_analyzer": "ik_smart"   // 使用ik_smart分词器，在查询文档数据的时，使用这个分词器处理name属性数据，如果不用设置，那么默认使用analyzer设置的分词器
      }
    }
  }
}
...
// 酒店数据索引库，执行下面的命令就会创建名为hotel的索引库，即hotel的索引库配置
PUT /hotel
{
  "settings": {
    "analysis": {
      "analyzer": {
        "text_anlyzer": {             // 自定义了一个名为text_anlyzer的分词器
          "tokenizer": "ik_max_word", 
          "filter": "py"
        },
        "completion_analyzer": {      // 自定义了了一个名为completion_analyzer的分词器
          "tokenizer": "keyword",
          "filter": "py"
        }
      },
