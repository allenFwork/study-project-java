...
接下来，利用Navicat客户端连接MySQL，然后导入special_data目录下提供的sql文件：
...
之所以将库存分离出来，<font color=red>是因为库存是更新比较频繁的信息，写操作较多</font>。而其他信息修改的频率非常低。

# 3. 导入Demo工程

查看ms-cache-service的工程 

项目结构如图所示：

![](C:\Users\shiwei\AppData\Roaming\marktext\images\2024-02-05-16-49-20-image.png)
...
在`com.study.cache.web`包的`ItemController`中可以看到接口定义：
...
其中的 192.168.150.101 是我的虚拟机IP，也就是我的Nginx业务集群要部署的地方：
