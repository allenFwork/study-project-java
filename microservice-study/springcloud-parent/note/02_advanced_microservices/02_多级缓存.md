...
**Caffeine**是一个基于Java8开发的，提供了近乎最佳命中率的高性能的本地缓存库。目前Spring内部的缓存使用的就是Caffeine。[GitHub地址]([GitHub - ben-manes/caffeine: A high performance caching library for Java](https://github.com/ben-manes/caffeine)) 
...
Lua声明变量的时候无需指定数据类型，而是用 **local来声明变量为局部变量**：
...
安装Lua可以参考note目录下的 `安装OpenResty.md` 文档：
...
<img title="" src="file:///C:/Users/shiwei/AppData/Roaming/marktext/images/2024-02-06-10-08-12-image.png" alt="" width="671">
...
- `~` 表示接下来使用正则表达式；`()` 表示对正则表达式分组； `\d+` 其中“\d”表示数字，“+”表示至少有一位，整体表示至少有一位数字
...
upstream tomcat-cluster {
    # nginx默认使用的是轮询策略，此处修改为基于哈希算法的负载均衡（对请求路径进行哈希运算，从而选择一个节点）
    hash $request_uri;
