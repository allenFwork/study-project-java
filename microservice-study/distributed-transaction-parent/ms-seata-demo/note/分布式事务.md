...
特殊情况：对同一张表同一个字段进行操作的两个事务，一个是由Seata管理的，一个无关，此时执行的情况如下：

![](C:\Users\shiwei\AppData\Roaming\marktext\images\2024-02-02-14-41-58-image.png)
...
导入special_data目录中的Sql文件：seata-at.sql，其中lock_table导入到TC服务关联的数据库，undo_log表导入到微服务关联的数据库：
...
- 不依赖数据库事务，而是依赖补偿操作，<font color=red>可以用于非事务型数据库</font>
...
public interface AccountTCCService {
     ...
     * 使用 @BusinessActionContextParameter 注解的参数，在 confirm 和 cancel 状态对应的方法中可以通过BusinessActionContext类型对象获取
     */
    @TwoPhaseBusinessAction(name = "deduct", commitMethod = "confirm", rollbackMethod = "cancel")
    void deduct(@BusinessActionContextParameter(paramName = "userId") String userId,
                @BusinessActionContextParameter(paramName = "money")int money);

    // 通过 BusinessActionContext 可以获取到事务信息和参数信息
...
public class AccountTCCServiceImpl implements AccountTCCService {
    ...
    public void deduct(String userId, int money) {
        ...
        // 补充：业务悬挂问题，判断freeze中是否由有冻结记录，如果有，那么一定执行过Cancel了，就要拒绝业务
        ...
        // 1.扣减可用余额 (因为表中余额字段类型为unsigned，无符号数类型，所以扣减小于0时会自动报错)
    ...
    public boolean confirm(BusinessActionContext ctx) {
        ...
        return count == 1; // 理论上删除1条记录就是成功了
    }

    @Override
    public boolean cancel(BusinessActionContext ctx) {
        // 0.查询冻结记录
        String xid = ctx.getXid();     // 通过BusinessActionContext获取事务ID
        AccountFreeze freeze = freezeMapper.selectById(xid); // 查询表获取信息

        // 补充1：空回滚的判断，判断freeze是否为null，为null证明try没有执行，需要空回滚
        if(freeze == null) {
            // 证明try没执行，需要空回滚
        ...
        // 补充2：幂等判断(由于一些问题,cancel执行时间超长，TC又进行了一次cancel请求，得保证结果一致，不能再执行以下的修改余额逻辑)
